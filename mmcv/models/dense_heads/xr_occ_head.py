...
class OccHead_XR(BaseModule):
    ...
    def forward(self, x, ins_query):
        ...
        return mask_preds, ins_occ_logits, cur_state, ins_occ_query

    def merge_queries(self, outs_dict, detach_query_pos=True):
        traj_query = outs_dict.get('traj_query', None)       # [n_dec, b, nq, n_modes, dim]
        ...
        if traj_query is not None:
            traj_query = traj_query[-1]
            traj_query  = self.mode_fuser(traj_query ).max(2)[0]
            base_query = traj_query
        else:
            # 没有轨迹信息 ⇒ 直接用 track_query 作为基础
            base_query = track_query                          # [b, nq, d]

        ins_query = self.multi_query_fuser(torch.cat([base_query, track_query, track_query_pos], dim=-1))
    ...
    def forward_test(
        ...
        if no_query:
            ...
            ins_occ_query = torch.zeros((1, 5, 5, 256),device=bev_feat.device)
            cur_state = None
            return out_dict, cur_state, ins_occ_query


        ins_query = self.merge_queries(outs_dict, self.detach_query_pos)
        # pred_ins_logits 表示实例分割row logits
        _, pred_ins_logits, cur_state, ins_occ_query = self(bev_feat, ins_query=ins_query)
        ...
        return out_dict, cur_state, ins_occ_query # cur_state [625(25*25) 1 256], ins_occ_query [1, 5, 12, 256]
