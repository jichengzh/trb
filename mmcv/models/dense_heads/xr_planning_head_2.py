...
class PlanningHeadSingleMode_XR_seg(nn.Module):
    def __init__(self,
                 bev_h=200,
                 bev_w=200,
                 embed_dims=256,
        ...
        """
        super(PlanningHeadSingleMode_XR_seg, self).__init__()
    ...
    def forward_test(self, bev_embed, outs_motion=None, outs_occflow=None, command=None, lane_query=None, ins_occ_query=None):
        # outs_motion = outs_motion or {}
        # outs_occflow = outs_occflow or {}
        ...
        # 获取 motion 分支输出，若无则生成默认全 0 Tensor
        sdc_traj_query = outs_motion.get('sdc_traj_query', torch.zeros(3, 1, 6, 256, device=device))
        sdc_track_query = outs_motion.get('sdc_track_query', torch.zeros(1, 256, device=device))
        bev_pos = outs_motion.get('bev_pos', None)  # 这个不默认填充，可能依赖模型输出结构
        occ_mask = outs_occflow.get('seg_out', None)

        # 执行主规划前向流程
        outs_planning = self(bev_embed, occ_mask, bev_pos, sdc_traj_query, sdc_track_query, command, lane_query, ins_occ_query)
    ...
    def forward(self, 
        ...
        """
        target_len = 6
        B, L, C = lane_query.shape

        sdc_track_query = sdc_track_query.detach()
        # sdc_traj_query[-1]会丢弃第一个维度
        ...
        # seg_len = L // target_len
        # lane_query = lane_query[:, :seg_len * target_len, :]          # 裁掉无法整分的尾巴
        # lane_query = lane_query.view(B, target_len, seg_len, C).mean(2)
    
        # print('sdc_track_query', sdc_track_query.shape)
        # print('navi_embed', navi_embed.shape)
        # print('sdc_traj_query', sdc_traj_query.shape)
        # plan_query = torch.cat([sdc_traj_query, sdc_track_query, navi_embed], dim=-1)
        # =================== 修改 ========================================
        if ins_occ_query is not None and lane_query is None:
            B, T, Q, C = ins_occ_query.shape
            # 压缩实例维度
            occ_feat = ins_occ_query.mean(dim=2) #[1,5,5,256]  →  [1,5,256]
            # 2) 调整时间维到 P=6
            if T < P:                                 # 需要补帧
                pad = occ_feat[:, -1:, :].expand(-1, P - T, -1)
                occ_feat = torch.cat([occ_feat, pad], dim=1)        # [1, 6, 256]
            elif T > P:                               # 需要裁剪
                occ_feat = occ_feat[:, :P]            # 保留前 P 帧
            
            plan_query = torch.cat([sdc_track_query, navi_embed, occ_feat], dim=-1)
        # ==================== 修改 =======================================     
        elif lane_query is not None:
            lane_query = lane_query[:, :target_len, :]
            plan_query = torch.cat([sdc_track_query, navi_embed, lane_query], dim=-1)
