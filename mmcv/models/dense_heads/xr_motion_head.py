...
class MotionHead_XR(BaseMotionHead):
    """
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MotionHead_XR, self).__init__()
    ...
    def forward_test(self, bev_embed, outs_track={}, outs_seg={}):
        ...
        track_boxes[0][3] = torch.cat([track_boxes[0][3], sdc_track_boxes[0][3]], dim=0)      
        # ================  修改，消融seg ==============================
        if outs_seg is not None:
            memory, memory_mask, memory_pos, lane_query, _, lane_query_pos, hw_lvl = outs_seg['args_tuple']
        else:
            B, D = track_query.shape[0], track_query.shape[-1]
            lane_query      = track_query.new_zeros(B, 0, D)          # shape=(B, 0, D) 第二维是空的所有没有什么query
            lane_query_pos  = track_query.new_zeros(B, 0, D)  
        # ================ 修改，消融seg ================================
