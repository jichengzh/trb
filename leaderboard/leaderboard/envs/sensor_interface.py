...
class SensorInterface(object):
    ...
    def get_data(self, frame):
        ...
        print("[Queue size before get]", self._data_buffers.qsize())
        print("[Timeout]", self._queue_timeout)

        expected = set(self._sensors_objects.keys())
        # 不强制等待 opendrive 的情况要从 expected 里去掉
        if self._opendrive_tag:
            expected.discard(self._opendrive_tag)
        seen = set()
        ...
        try:
            ...
            while len(data_dict.keys()) < len(self._sensors_objects.keys()):
                ...
                print("[Got data]", sensor_data[0], sensor_data[1])
                if sensor_data[1] != frame:
                    print("[Frame mismatch]", sensor_data[1], frame)
                    continue
                    
                data_dict[sensor_data[0]] = ((sensor_data[1], sensor_data[2]))

            while len(data_dict.keys()) < len(self._sensors_objects.keys()):
                # 如果只剩下 opendrive，则按你原逻辑提前跳出
                if self._opendrive_tag and self._opendrive_tag not in data_dict.keys() \
                        and len(self._sensors_objects.keys()) == len(data_dict.keys()) + 1:
                    break

                sensor_data = self._data_buffers.get(True, self._queue_timeout)
                sid, sframe, svalue = sensor_data
                print("[Got data]", sid, sframe)

                if sframe != frame:
                    print("[Frame mismatch]", sid, "got", sframe, "expect", frame)
                    continue

                # 收到“当前帧”的有效数据后再更新 data_dict / seen
                data_dict[sid] = (sframe, svalue)

                # === DEBUG: 标记已收到并打印缺失者 ===
                seen.add(sid)
                missing = expected - seen
                print("[missing now]", missing)
