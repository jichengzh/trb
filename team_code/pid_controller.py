...
class PIDController(object):
    ...
    def __init__(self, turn_KP=0.75, turn_KI=0.75, turn_KD=0.3, turn_n=40, speed_KP=5.0, speed_KI=0.5,speed_KD=1.0, speed_n = 40,max_throttle=0.75, brake_speed=0.4,brake_ratio=1.1, clip_delta=0.25, aim_dist=4.0, angle_thresh=0.3, dist_thresh=10):
        ...
        # PID中增加KP可以提高PID的灵敏度
        # PID中增加KI可以提高PID的稳定性
        # PID中增加KD可以提高PID的响应速度
        # self.turn_controller = PID(K_P=turn_KP+0.25, K_I=turn_KI-0.25, K_D=turn_KD, n=turn_n)
        ...
        # 提高最大油门输出限制
        # self.max_throttle = max_throttle+0.1
        self.max_throttle = max_throttle
        # brake速度表示如果期望速度低于这个值，则会触发刹车
        self.brake_speed = brake_speed
        # brake_ratio表示如果当前速度与期望速度的比值大于这个值，则会触发刹车
        # self.brake_ratio = brake_ratio+0.4
        self.brake_ratio = brake_ratio
        # clip_delta表示期望速度与当前速度之间的差值，范围在0.0到clip_delta之间
        # self.clip_delta = clip_delta+2
        self.clip_delta = clip_delta
        self.aim_dist = aim_dist
        # 增大最大角接受范围允许更快拐弯
        # self.angle_thresh = angle_thresh+0.2
    ...
    def control_pid(self, waypoints, speed, target):
        ...
        desired_speed = 0
        aim = waypoints[0]
        # 这个for循环的作用是计算期望速度和最佳航迹点
        for i in range(num_pairs):
            # magnitude of vectors, used for speed, 计算期望速度航迹点之差乘以2,最终 desired_speed 是 5 段距离的加权平均，是 轨迹的粗略平均速度估计。
        ...
        #       desired_speed += min(3.5, 0.4 * (desired_speed - speed))
        ...
        # delta表示期望速度与当前速度之间的差值，范围在0.0到clip_delta之间
        # clip函数的作用是将delta限制在0.0到clip_delta之间，避免过大的加速
        delta = np.clip(desired_speed - speed, 0.0, self.clip_delta)
        # throttle表示车辆的油门控制，范围在0.0到max_throttle之间
