...
# 提取时间戳和metaxin'xi
    for sample_id, data in tqdm(enumerate(data_loader)):
        ...
        # 构建onnx/tensorrt推理需要输入的字段。先塞入data已有字段。
        onnx_inputs = dict()
        ...
        if shape0 is None:
            ...
            if shape0 == onnx_inputs['prev_track_intances0'].shape[0]:
                for key in onnx_input_shapes.keys():
                    arr = onnx_inputs[key].detach().cpu().numpy() if key in onnx_inputs \
                        else np.zeros(onnx_input_shapes[key], dtype=np.float32)  # 或者你自己的占位逻辑

                    if key not in npz_data:
                        # key不在onnx_inputs中，这里就填充
                        npz_data[key] = arr
                    else:
                        # key在npz_data中已经存在，这里就拼接
                        npz_data[key] = np.concatenate([npz_data[key], arr], axis=0)
                np.savez('/home/featurize/DL4AGX/AV-Solutions/uniad-trt/UniAD/calib_data_shape0_'+str(shape0)+'.npz', **npz_data)
                num_calib_data += 1
