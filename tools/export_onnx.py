...
def main():
    ...
    output_shapes = dict(
        ...
        obj_idxes=[-1],
        max_obj_id_out=[1],
        seg_out=[1, 5, 1, bevh*2, bevh*2], # preserved for future collision optimization use
        outs_planning=[1, 6, 2],
    )
    ...
    for iid in tqdm(range(6)):
        inputs = {}
        # ======= 1. 逐个键读取 / 生成输入张量 ===============
        for key in input_shapes.keys():    # input_shapes里罗列模型需要的所有输入名
            if os.path.isfile(onnx_export_input+key+'/'+str(iid)+'.npy'): # 加载input中的元数据一般是相机内参，真实值等
                inputs[key]= np.load(onnx_export_input+key+'/'+str(iid)+'.npy')  # 加载输入数据
                inputs[key] = torch.from_numpy(inputs[key]).cuda()
                if key=='timestamp': # 对于时间戳进行特殊处理
                    if iid==0:
                        import copy
                        timestamp0=copy.deepcopy(inputs[key]) # 需要记录第0帧的绝对时间
                    inputs[key] = (inputs[key] - timestamp0).float()
            else: 
                if key=='image_shape':# 记录原始输入图像（或 resize 后）高宽，供模型中 resize / position 编码。
                    ...
                    print('在这里prev_bev的尺寸为', inputs[key].shape)
                    seq_len, B, C = inputs[key].shape
                    if seq_len != bevh**2:
                        bev_map = inputs[key].permute(1, 2, 0).reshape(B, C, bevh*2, bevh*2)
                        pool = nn.AvgPool2d(kernel_size=2, stride=2)
                        bev_map_small = pool(bev_map)
                        h2, w2 = bev_map_small.shape[-2:]
                        inputs[key] = bev_map_small.reshape(B, C, h2 * w2).permute(2, 0, 1).contiguous()
                    else:
                        pass
        ...
        max_obj_id = torch.from_numpy(np.load('/home/featurize/DL4AGX/AV-Solutions/uniad-trt/UniAD/nuscenes_np/uniad_pth_trtp_out/max_obj_id_out'+'/'+str(iid)+'.npy')).cuda()
        prev_l2g_r_mat_out = torch.from_numpy(np.load('/home/featurize/DL4AGX/AV-Solutions/uniad-trt/UniAD/nuscenes_np/uniad_pth_trtp_out/prev_l2g_r_mat_out'+'/'+str(iid)+'.npy')).cuda()
        prev_l2g_t_out = torch.from_numpy(np.load('/home/featurize/DL4AGX/AV-Solutions/uniad-trt/UniAD/nuscenes_np/uniad_pth_trtp_out/prev_l2g_t_out'+'/'+str(iid)+'.npy')).cuda()
        prev_timestamp_out = torch.from_numpy(np.load('/home/featurize/DL4AGX/AV-Solutions/uniad-trt/UniAD/nuscenes_np/uniad_pth_trtp_out/prev_timestamp_out'+'/'+str(iid)+'.npy')).cuda()
