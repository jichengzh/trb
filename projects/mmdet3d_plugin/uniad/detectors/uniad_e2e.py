...
class UniADTRT(UniADTrackTRT):
    ...
    def forward_uniad_trt(self,
        ...
        tmp = {
            "bev_embed": bev_embed,          # (10000, B, 256)  or (40000, B, 256)
            "bev_pos"  : bev_pos,            # 如果你后面要用的话
            "prev_bev": prev_bev           # 若后续还会用到
        }

        tmp = self.upsample_bev_if_tiny(tmp)  # ← 内部若检测 100×100 就 ×2
        print('bev_embed shape:', bev_embed.shape) 
        bev_embed = tmp["bev_embed"]          # (40000, B, 256)  for tiny
        bev_pos   = tmp.get("bev_pos", bev_pos)
        prev_bev = tmp["prev_bev"]
        print('-------------------------------------------------------------------------------')
        print("prev_bev 的尺寸是", prev_bev.shape)
